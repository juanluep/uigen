...
vi.mock("../ToolCallBadge", () => ({
  ToolCallBadge: ({ toolInvocation }: { toolInvocation: { toolName: string; args: Record<string, unknown>; state: string } }) => (
    <div data-testid="tool-call-badge">{`${toolInvocation.toolName}:${toolInvocation.args.command ?? ""}`}</div>
  ),
}));
